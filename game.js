...
function playRound(playerSelection, computerSelection) {
  ...
  if (result == "You Win! ") {
    return [
      result + playerSelection + beats + computerSelection + point,
      "Win",
    ];
  } else {
    return [
      result + computerSelection + beats + playerSelection + point,
      "Lose",
    ];
  }
}
...
function playerPlay() {
  let playerSelection = prompt(
    "Type between the game option (Rock, Scisor, Paper):"
  );
